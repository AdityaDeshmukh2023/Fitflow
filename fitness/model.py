...
class FitnessProfile(models.Model):
    ...
    activity_level = models.CharField(
        max_length=20, choices=ACTIVITY_LEVEL_CHOICES, help_text="Daily activity level"
    )
    goal = models.CharField(
        max_length=50, choices=GOAL_CHOICES, help_text="Primary fitness goal"
    )
    ...
    target_timeline = models.CharField(
        max_length=50, help_text="Target timeline for achieving your goal"
    )

    # Health Considerations
    medical_conditions = models.TextField(
        blank=True, help_text="Any medical conditions that might affect your exercise"
    )
    injuries_or_physical_limitation = models.TextField(
        blank=True, help_text="Any injuries or physical limitations to consider"
    )
    ...
    available_equipment = models.TextField(
        blank=True, help_text="List any exercise equipment you have access to"
    )

    # Lifestyle Factors
    sleep_pattern = models.CharField(
        max_length=20, choices=SLEEP_PATTERN_CHOICES, help_text="Average daily sleep"
    )
...
class FitnessRecommendation(models.Model):
    ...
    recommendation_json = models.JSONField(null=True)
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def __str__(self):
        return (
            f"Fitness Recommendation for {self.profile.goal} - {self.created_at.date()}"
        )
